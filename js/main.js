...
import { doc, addDoc, setDoc, deleteDoc, onSnapshot, collection, serverTimestamp, updateDoc } from "https://www.gstatic.com/firebasejs/11.6.1/firebase-firestore.js";
...
// Se corrigió la lógica de navegación para asegurar que se activen los eventos.
...
// Manejo de autenticación para el entorno.
onAuthStateChanged(auth, async (user) => {
    if (user) {
        ...
        try {
            // Intenta iniciar sesión anónimamente si no hay un token personalizado.
            await signInAnonymously(auth);
        } catch (error) {
            console.error("Error al firmar anónimamente:", error);
            showModal("Error al iniciar sesión. Por favor, recarga la página.");
        }
    }
});
...
function setupRealtimeListeners() {
    ...
    onSnapshot(salesCollection, (snapshot) => {
        ...
        updateDailyTotals(allSales, 'sales');
    ...
    onSnapshot(expensesCollection, (snapshot) => {
        ...
        updateDailyTotals([], 'expenses', expenses);
    }, (error) => {
    ...
    const cashDocRef = doc(db, `artifacts/${appId}/users/${userId}/cajas`, today);
    ...
    const cashHistoryCollection = collection(db, `artifacts/${appId}/users/${userId}/cajas_historico`);
    onSnapshot(cashHistoryCollection, (snapshot) => {
        const history = snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }));
        renderCashHistory(history);
    });

    const paymentMethodsCollection = collection(db, `artifacts/${appId}/users/${userId}/paymentMethods`);
    ...
    const expenseCategoriesCollection = collection(db, `artifacts/${appId}/users/${userId}/expenseCategories`);
    onSnapshot(expenseCategoriesCollection, (snapshot) => {
        userExpenseCategories = snapshot.docs.map(doc => doc.data().name);
        renderExpenseCategories();
    });

    const customersCollection = collection(db, `artifacts/${appId}/users/${userId}/customers`);
    ...
    // NUEVOS LISTENERS para la página de Configuración
    const usersConfigCollection = collection(db, `artifacts/${appId}/users/${userId}/users_config`);
    onSnapshot(usersConfigCollection, (snapshot) => {
        allUsers = snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }));
        renderUsersList(allUsers);
    });

    const rolesCollection = collection(db, `artifacts/${appId}/users/${userId}/roles`);
    onSnapshot(rolesCollection, (snapshot) => {
        allRoles = snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }));
        renderRolesList(allRoles);
        renderUserRolesSelect(allRoles);
    });
}
...
function renderSalesHistory(sales) {
    ...
    const salesByDay = filteredSales.reduce((acc, sale) => {
        const date = new Date(sale.timestamp.seconds * 1000).toLocaleDateString('es-ES', { year: 'numeric', month: 'long', day: 'numeric' });
        if (!acc[date]) {
...
function renderDailyExpenses(expenses) {
    ...
    expenses.forEach(expense => {
        ...
        expenseDiv.className = "bg-white p-3 rounded-lg shadow-sm flex justify-between items-center";
        ...
        `;
        dailyExpensesContainer.appendChild(expenseDiv);
    });
}
...
function updateDailyTotals(sales, type, expenses = []) {
    if (!dailyCashData) return;

    if (type === 'sales') {
        dailySalesTotal = sales.reduce((sum, sale) => {
            const saleDate = new Date(sale.timestamp.seconds * 1000).toLocaleDateString('en-CA');
            const today = new Date().toLocaleDateString('en-CA');
            if (saleDate === today) {
    ...
    if (type === 'expenses') {
        dailyExpensesTotal = expenses.reduce((sum, expense) => sum + expense.amount, 0);
    }
...
processPaymentBtn.addEventListener('click', async () => {
    ...
    try {
        const productUpdates = cart.map(item => {
            const productDocRef = doc(db, `artifacts/${appId}/users/${userId}/products/${item.id}`);
...
function exportSalesToCsv(sales) {
    ...
    const rows = sales.map(sale => {
        const date = new Date(sale.timestamp.seconds * 1000).toLocaleString('es-ES');
        ...
        item.querySelector('.delete-customer-btn').addEventListener('click', async () => {
            if (confirm(`¿Estás seguro de que quieres eliminar a ${customer.name}?`)) {
                try {
                    ...
                    showModal("Error al eliminar cliente. Intenta de nuevo.");
                }
            }
        });
    });
...
// Lógica de la página de Configuración (NUEVA)
addUserForm.addEventListener('submit', async (e) => {
    e.preventDefault();
    const username = usernameInput.value.trim();
    const role = userRoleSelect.value;
    if (!username || !role) {
        showModal("El nombre de usuario y el rol son obligatorios.");
        return;
    }

    try {
        const usersConfigCollection = collection(db, `artifacts/${appId}/users/${userId}/users_config`);
        await addDoc(usersConfigCollection, { username, role });
        addUserForm.reset();
        showModal(`Usuario '${username}' con rol '${role}' añadido con éxito.`);
    } catch (error) {
        console.error("Error al añadir usuario:", error);
        showModal("Error al añadir usuario. Intenta de nuevo.");
    }
});
...
addRoleForm.addEventListener('submit', async (e) => {
    ...
    if (!roleName) {
        showModal("El nombre del rol no puede estar vacío.");
        return;
    }

    try {
        const rolesCollection = collection(db, `artifacts/${appId}/users/${userId}/roles`);
        await addDoc(rolesCollection, { name: roleName });
        addRoleForm.reset();
        showModal(`Rol '${roleName}' añadido con éxito.`);
    } catch (error) {
        console.error("Error al añadir rol:", error);
        showModal("Error al añadir rol. Intenta de nuevo.");
    }
});
...
function renderUsersList(users) {
    ...
    users.forEach(user => {
        ...
        item.className = "bg-gray-100 p-3 rounded-lg flex justify-between items-center";
        item.innerHTML = `
            <span>${user.username} - <span class="font-semibold">${user.role}</span></span>
            <button data-id="${user.id}" class="delete-user-btn px-3 py-1 bg-red-500 text-white rounded-lg text-sm">
                <i class="fas fa-trash-alt"></i> Eliminar
            </button>
        `;
        usersListContainer.appendChild(item);

        item.querySelector('.delete-user-btn').addEventListener('click', async () => {
            if (confirm(`¿Estás seguro de que quieres eliminar al usuario '${user.username}'?`)) {
                try {
                    const userDocRef = doc(db, `artifacts/${appId}/users/${userId}/users_config/${user.id}`);
                    await deleteDoc(userDocRef);
                    showModal("Usuario eliminado.");
                } catch (error) {
                    console.error("Error al eliminar usuario:", error);
                    showModal("Error al eliminar usuario. Intenta de nuevo.");
                }
            }
        });
    });
...
function renderRolesList(roles) {
    ...
    roles.forEach(role => {
        ...
        item.innerHTML = `
            ...
            <button data-id="${role.id}" class="delete-role-btn px-3 py-1 bg-red-500 text-white rounded-lg text-sm">
                <i class="fas fa-trash-alt"></i> Eliminar
            </button>
        `;
        rolesListContainer.appendChild(item);
        ...
        item.querySelector('.delete-role-btn').addEventListener('click', async () => {
            if (confirm(`¿Estás seguro de que quieres eliminar el rol '${role.name}'?`)) {
                try {
                    const roleDocRef = doc(db, `artifacts/${appId}/users/${userId}/roles/${role.id}`);
                    await deleteDoc(roleDocRef);
                    showModal("Rol eliminado.");
                } catch (error) {
                    console.error("Error al eliminar rol:", error);
                    showModal("Error al eliminar rol. Intenta de nuevo.");
                }
            }
        });
    });
...
function renderUserRolesSelect(roles) {
...
// Asegúrate de que esta llamada está al final del script para que se ejecute una vez que se cargue la página
