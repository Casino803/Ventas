...
// Se elimina la función setupNavigation obsoleta.
...
function renderReservations() {
    ...
    document.querySelectorAll('.process-reservation-btn').forEach(btn => {
        btn.addEventListener('click', (e) => {
            // ✅ VERIFICACIÓN DE CAJA AÑADIDA (1 de 2: Facturación de Reserva)
            if (!dailyCashData || dailyCashData.cerrada) {
                showModal("🚨 ERROR: No se puede facturar la reservación. Debe abrir la caja para registrar la venta.");
                return;
            }
...
function showPaymentModalForReservation(reservation) {
    ...
    cart = reservation.items.map(item => {
        // Asegurar que el carrito se reconstruya con la información completa del producto/combo
        if (item.isCombo) {
            const combo = allCombos.find(c => c.id === item.comboId);
            return {
                ...item,
                stock: undefined, // El stock se maneja a nivel de producto individual
                basePrice: combo.items.reduce((sum, comboItem) => {
                    const product = allProducts.find(p => p.id === comboItem.productId);
                    return sum + (product ? product.price * comboItem.quantity : 0);
                }, 0),
            };
        } else {
    ...
    // Aplicar descuento/recargo de la reserva
    if (reservation.adjustment && reservation.adjustment.amount > 0) {
        const { value, type } = calculateDiscountValueAndType(reservation.subtotal, reservation.total, reservation.adjustment.amount, reservation.adjustment.type);
        currentDiscountSurcharge.value = value;
        currentDiscountSurcharge.type = type;
        
        if (discountSurchargeValueInput) discountSurchargeValueInput.value = value;
        if (discountSurchargeTypeSelect) discountSurchargeTypeSelect.value = type;
    } else {
...
// Función auxiliar para recalcular el valor del descuento/recargo basado en subtotal/total/ajuste
function calculateDiscountValueAndType(subtotal, total, adjustmentAmount, adjustmentType) {
    let value = adjustmentAmount;
    if (adjustmentType.includes('percentage')) {
        const base = adjustmentType.includes('discount') ? subtotal : subtotal; // Usar subtotal para calcular el %
        if (base > 0) {
            value = (adjustmentAmount / base) * 100;
        } else {
            value = 0;
        }
    }

    return { value: parseFloat(value.toFixed(2)), type: adjustmentType };
}


// NUEVO: Función para procesar una reservación (OBSOLETA, AHORA SE HACE EN processPaymentBtn)
async function processReservedOrder(reservation, payments) {
    // Esta función ya no se usa, la lógica se movió a processPaymentBtn para unificar el flujo de pago.
}

// NUEVO: Función para eliminar una reservación
async function deleteReservedOrder(reservationId) {
    // Buscar la reservación antes de eliminarla para obtener los items
    const reservationToDelete = allReservations.find(r => r.id === reservationId);
    ...
    showConfirmationModal(`¿Estás seguro de que quieres eliminar esta reservación? Esta acción no se puede deshacer.`, async () => {
        try {
            ...
            if (reservationToDelete && reservationToDelete.items) {
                const stockRevertUpdates = reservationToDelete.items.map(item => {
                    if (item.isCombo && item.items) {
                        // Devolver stock de los componentes del combo
                        return item.items.map(comboItem => {
                            ...
                            return updateDoc(productDocRef, {
                                stock: increment(comboItem.quantity * item.quantity) // Sumar stock
                            });
                        });
                    }
                    if (!item.isCombo && item.stock !== undefined) {
                        // Devolver stock de productos individuales
                        const productDocRef = doc(db, SHARED_PRODUCTS_COLLECTION, item.id);
                        return updateDoc(productDocRef, {
                            stock: increment(item.quantity) // Sumar stock
                        });
                    }
                }).flat().filter(Boolean);

                await Promise.all(stockRevertUpdates);
            }
            ...
            showModal("Reservación eliminada y stock devuelto con éxito.");
        } catch (error) {
...
function setupRealtimeListeners() {
    ...
    onSnapshot(salesCollection, (snapshot) => {
        ...
        renderSalesHistory(allSales); // Llamada para renderizar
    ...
    onSnapshot(expensesCollection, (snapshot) => {
        ...
        // La función renderDailyExpenses ya existe y renderiza el historial cronológico
    ...
    onSnapshot(cashDocRef, (doc) => {
        ...
        // ✅ CORRECCIÓN DE CAJA: Llamar a updateDailyTotals aquí para recargar los datos
        updateDailyTotals();
    }, (error) => {
    ...
    onSnapshot(cashHistoryCollection, (snapshot) => {
        ...
        renderCashHistory(history); // Llamada para renderizar
    }, (error) => {
...
function renderSalesHistory(sales) {
    ...
    // ✅ CORRECCIÓN 1/3: Ordenar las fechas de forma descendente (más reciente primero)
    const sortedDates = Object.keys(salesByDay).sort((a, b) => new Date(b) - new Date(a));
    
    // SE ELIMINÓ EL RECUADRO AZUL 'TOTAL FILTRADO'
    ...
    sortedDates.forEach(dateString => {
        ...
        // Ordenar las ventas dentro del día por timestamp descendente
        dailySales.sales.sort((a, b) => b.timestamp.seconds - a.timestamp.seconds);
...
function renderDailyExpenses(expenses) {
    ...
    // ✅ CORRECCIÓN 2/3: Ordenar las fechas de forma descendente (más reciente primero)
    ...
    sortedDates.forEach(dateString => {
        ...
        // Ordenamos los gastos dentro del día de forma descendente (más reciente primero)
        dayData.expenses.sort((a, b) => b.timestamp.seconds - a.timestamp.seconds); 
...
function renderCashHistory(history) {
    ...
    const historyByDay = history.reduce((acc, entry) => {
        // Utilizamos la fecha de apertura (entry.fecha) para agrupar
    ...
    // ✅ CORRECCIÓN 3/3: Ordenar las fechas de forma descendente (más reciente primero)
    ...
    sortedDates.forEach(dateString => {
        ...
        // Ordenamos los cierres de caja dentro del día por el timestamp de cierre (cierreTimestamp)
        historyByDay[dateString].sort((a, b) => b.cierreTimestamp.seconds - a.cierreTimestamp.seconds);

        const entriesHtml = historyByDay[dateString].map(entry => {
            totalSalesDay += entry.ventasTotales;
            totalExpensesDay += entry.gastosTotales;
            // Utilizamos el timestamp de cierre para mostrar la hora (más preciso)
            const entryDate = entry.cierreTimestamp ? new Date(entry.cierreTimestamp.seconds * 1000).toLocaleTimeString('es-ES') : new Date(entry.fecha.seconds * 1000).toLocaleTimeString('es-ES');
            return `
...
if (expenseForm) {
    expenseForm.addEventListener('submit', async (e) => {
        ...
        // ✅ VERIFICACIÓN AÑADIDA (2 de 2: Registro de Gasto)
        if (!dailyCashData || dailyCashData.cerrada) {
            showModal("🚨 ERROR: No se puede registrar el gasto. La caja debe estar abierta.");
            return;
        }
        ...
        try {
            ...
            // FIX: Actualizar los totales de caja después de registrar un gasto
...
async function processImportedSales(csvData) {
    ...
    for (let i = 1; i < rows.length; i++) {
        // FIX: Corregido el manejo de mensajes de error
    ...
    <div id="print-area">
        ...
        <div class="receipt-body">
            ...
            <div style="text-align: right; margin-top: 20px;">
                ${sale.subtotal && sale.subtotal !== sale.total ? `<p>Subtotal: $${sale.subtotal.toFixed(2)}</p>` : ''}
                ${sale.adjustment && sale.adjustment.amount > 0 ? `<p>${sale.adjustment.type.includes('discount') ? 'Descuento' : 'Recargo'}: ${sale.adjustment.type.includes('discount') ? '-' : '+'}$${sale.adjustment.amount.toFixed(2)}</p>` : ''}
...
if (showComboFormBtn) {
    showComboFormBtn.addEventListener('click', () => {
        if (addComboForm.classList.contains('hidden')) {
            addComboForm.classList.remove('hidden');
            comboIdInput.value = '';
            addComboForm.reset();
            comboProductsContainer.innerHTML = '';
            addComboProductInput(); // Agregar un campo por defecto
        } else {
...
// Lógica de alternancia de formularios (Asegurada dentro de DOMContentLoaded)
// if (toggleProductFormBtn && productFormContainer) {
//     toggleProductFormBtn.addEventListener('click', () => {
//         productFormContainer.classList.toggle('hidden');
//     });
// }

// if (toggleExpenseFormBtn && expenseFormContainer) {
//     toggleExpenseFormBtn.addEventListener('click', () => {
//         expenseFormContainer.classList.toggle('hidden');
//     });
// }

// if (toggleCustomerFormBtn && customerFormContainer) {
...
document.addEventListener('DOMContentLoaded', () => {
    // FIX: Mover los listeners de los botones de alternancia DENTRO de DOMContentLoaded
    if (toggleProductFormBtn && productFormContainer) {
        toggleProductFormBtn.addEventListener('click', () => {
            productFormContainer.classList.toggle('hidden');
        });
    ...
    // FIN FIX de botones de alternancia
    
    // setupNavigation(); // Se elimina la llamada a la función obsoleta
    setupTabNavigation();
    setupCashTabNavigation();

    // Lógica para los botones de navegación superiores (Configuración/Logout)
    const topNavButtons = document.querySelectorAll('.fixed button[data-page]');
    topNavButtons.forEach(btn => {
        btn.addEventListener('click', () => {
            const targetPageId = btn.dataset.page;
            showPage(targetPageId);
            // Si el botón no es una pestaña principal (como el de Configuración), 
            // aseguramos que la pestaña activa inferior se desactive visualmente.
    ...
    // FIX: Listener para el botón de añadir cliente rápido
    const quickAddCustomerBtn = document.getElementById('quick-add-customer-btn');
    const quickAddCustomerModal = document.getElementById('quick-add-customer-modal');
    const quickAddCustomerCloseBtn = document.getElementById('quick-add-customer-close-btn');
    const quickAddCustomerFormContent = document.getElementById('quick-add-customer-form-content');

    if (quickAddCustomerBtn && quickAddCustomerModal) {
        // 1. Mostrar el modal y clonar el formulario de cliente
        quickAddCustomerBtn.addEventListener('click', () => {
            // Clonar el contenido del formulario de cliente (de la página de Gestión de Clientes)
            const formTemplate = document.getElementById('add-customer-form');
            if (formTemplate && quickAddCustomerFormContent) {
                // Limpiar contenido anterior
                quickAddCustomerFormContent.innerHTML = ''; 
                
                // Clonar y reasignar IDs para evitar conflictos
                const clonedForm = formTemplate.cloneNode(true);
                clonedForm.id = 'temp-quick-add-form';
                clonedForm.classList.remove('flex', 'space-x-2');
                
                const nameInput = clonedForm.querySelector('#customer-name-input');
                if (nameInput) nameInput.value = ''; // Limpiar input
                
                const submitButton = clonedForm.querySelector('button[type="submit"]');
                if (submitButton) {
                    submitButton.textContent = 'Guardar';
                    submitButton.classList.remove('flex-grow');
                }

                // Adjuntar el formulario clonado al modal
                quickAddCustomerFormContent.appendChild(clonedForm);
                
                // Añadir listener para el envío del formulario del modal
                clonedForm.addEventListener('submit', handleQuickAddCustomerSubmit);
                
                quickAddCustomerModal.classList.remove('hidden');
            }
        });

        // 2. Cerrar el modal
        if (quickAddCustomerCloseBtn) {
            quickAddCustomerCloseBtn.addEventListener('click', () => {
                quickAddCustomerModal.classList.add('hidden');
            });
        }
    }

    // 3. Función de manejo de envío para el modal rápido
    async function handleQuickAddCustomerSubmit(e) {
        e.preventDefault();
        
        // El input de nombre clonado es el primero
        const nameInput = e.target.querySelector('#customer-name-input');
        ...
        try {
            ...
            if (quickAddCustomerModal) quickAddCustomerModal.classList.add('hidden');
            showModal(`Cliente '${name}' añadido con éxito.`);
    ...
    // FIN FIX: Modal rápido de cliente
    ...
    if (processPaymentBtn) {
        processPaymentBtn.addEventListener('click', async () => {
            ...
            try {
                ...
                // Limpiamos el Carrito de propiedades no compatibles con Firestore (stock, basePrice)
                const cleanedCart = cart.map(item => {
                    // Clonamos el objeto para no modificar el carrito global
                    const cleanItem = { ...item }; 
                    
                    // Eliminamos incondicionalmente las propiedades que causan 'undefined'
                    delete cleanItem.stock;
                    delete cleanItem.basePrice;

                    // Aseguramos la limpieza en items anidados de combos (si existiera stock en ellos)
                    if (cleanItem.items && Array.isArray(cleanItem.items)) {
                        cleanItem.items = cleanItem.items.map(subItem => {
                            const cleanSubItem = { ...subItem };
                            delete cleanSubItem.stock;
                            return cleanSubItem;
                        });
                    }
                ...
                const saleData = {
                    items: cleanedCart, // Usamos el carrito limpio
                    subtotal: subtotal,
                    adjustment: {
                ...
                // 3. Procesar Venta / Reservación
                if (currentReservationToProcess) {
                    // Si es una reservación, el cliente se toma de la reserva para el historial, 
                    // aunque el total y los pagos se basan en el cálculo actual del modal de pago.
                    saleData.customerId = currentReservationToProcess.customerId;
                    saleData.customerName = currentReservationToProcess.customerName;

                    newSaleRef = await addDoc(collection(db, SHARED_SALES_COLLECTION), saleData);
                ...
                if (confirm("¿Deseas imprimir el recibo de la venta?")) {
                    printReceipt({
                        id: newSaleRef.id,
                        ...saleData,
                        customerName: saleData.customerName, // Aseguramos que el nombre del cliente se pase
                        timestamp: new Date()
                    });
                }
                
                // FIX: Forzar la actualización de los totales después de registrar la venta
                await updateDailyTotals();
    ...
    async function reserveOrder() {
        ...
        if (!customerId || customerName === 'Seleccionar Cliente') {
        ...
        try {
            ...
            renderCart();
            if (customerSelect) customerSelect.value = "";
            showModal("Pedido reservado con éxito y stock descontado.");
        } catch (error) {
            console.error("Error al reservar el pedido:", error);
            showModal("Hubo un error al reservar el pedido. Intenta de nuevo.");
            
            // Si la reservación falla, podrías considerar revertir el stock aquí 
            // si la falla no es por falta de stock. Sin embargo, para mantener 
            // la simplicidad, solo mostramos el error.
        }
    }
});